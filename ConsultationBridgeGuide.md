...
1. **Payment confirms** in Aira (`confirm_expert_handoff_payment`). The status
...
4. **Aira records** `astro_question_id` / `astro_horoscope_id` /
   `astro_user_id` on the session (migration 176). If the push failed, the
...
- `backend/app/services/expert_handoff.py` — payment confirm, reply delivery, reconcile job, `session_ref_to_id`
- `backend/app/routes/expert_handoff.py` — push hook after payment; public `/astro-reply` route
- `backend/app/main.py` — `astro-push-reconcile` scheduler job
- `backend/supabase/migrations/176_astro_bridge_session_links.sql`
- Tests: `backend/tests/test_astro_normalize.py`, `test_astro_bridge.py`, `test_astro_reply_hardening.py`, plus updates in `test_expert_handoff*.py`
