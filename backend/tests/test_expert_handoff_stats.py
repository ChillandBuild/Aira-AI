"""GET /api/v1/expert-handoff/stats — the Consultation Dashboard numbers."""
import sys
...
from app.routes.expert_handoff import expert_handoff_stats, require_conversations_view, router

app = FastAPI()
app.include_router(router, prefix="/api/v1/expert-handoff")
...
def test_stats_counts_messages_pending_answered_and_revenue():
    ...
    with patch("app.routes.expert_handoff.get_supabase", return_value=_db(rows)):
        res = client.get("/api/v1/expert-handoff/stats")
...
def test_stats_is_tenant_scoped_and_survives_an_empty_table():
    db = _db([])
    with patch("app.routes.expert_handoff.get_supabase", return_value=db):
        res = client.get("/api/v1/expert-handoff/stats")
...
def test_stats_reports_fractional_revenue_exactly():
    rows = [{"status": "paid", "amount_paise": 12345, "paid_at": _iso(0),
             "created_at": _iso(0), "astro_last_reply_id": None}]
    with patch("app.routes.expert_handoff.get_supabase", return_value=_db(rows)):
        body = client.get("/api/v1/expert-handoff/stats").json()
