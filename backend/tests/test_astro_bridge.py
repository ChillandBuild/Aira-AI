...
@pytest.mark.asyncio
async def test_push_followup_posts_followup_path():
    post = AsyncMock(return_value=_response({"success": True, "question_id": 9}))
    patcher, _ = _client_patch(post)
    try:
        with patch.object(astro_bridge, "get_setting", new=_settings_stub()):
            result = await astro_bridge.push_followup(_session(), _lead(), "  And my career?  ", TENANT, n=1)
    finally:
        patcher.stop()

    assert result["question_id"] == 9
    url, = post.call_args[0]
    assert url == "https://astro.example.com/api/astrologer-welcome/bridge/followup/"
...
@pytest.mark.asyncio
async def test_push_followup_uses_distinct_external_ref_per_followup():
    """Each follow-up gets its own idempotency key so Django never swallows it as the
    consultation's duplicate (CROSS-AGENT AGREEMENT: external_ref = session_id::f{n})."""
    post = AsyncMock(return_value=_response({"success": True, "question_id": 9}))
    patcher, _ = _client_patch(post)
    try:
        with patch.object(astro_bridge, "get_setting", new=_settings_stub()):
            await astro_bridge.push_followup(_session(), _lead(), "One more thing?", TENANT, n=1)
            first = post.call_args[1]["json"]["external_ref"]
            await astro_bridge.push_followup(_session(), _lead(), "And my career?", TENANT, n=2)
            second = post.call_args[1]["json"]["external_ref"]
    finally:
        patcher.stop()

    assert first == f"{SESSION_ID}::f1"
    assert second == f"{SESSION_ID}::f2"
    # Never collides with the consultation's key (the bare session id), nor with each other.
...
@pytest.mark.asyncio
@pytest.mark.parametrize("bad_n", [0, -1, None, "x"])
async def test_push_followup_refuses_invalid_followup_number(bad_n):
    post = AsyncMock()
    patcher, _ = _client_patch(post)
    try:
        with patch.object(astro_bridge, "get_setting", new=_settings_stub()):
            result = await astro_bridge.push_followup(_session(), _lead(), "A question", TENANT, n=bad_n)
    finally:
        patcher.stop()
...
@pytest.mark.asyncio
async def test_push_followup_refuses_empty_question():
    post = AsyncMock()
    patcher, _ = _client_patch(post)
    try:
        with patch.object(astro_bridge, "get_setting", new=_settings_stub()):
            result = await astro_bridge.push_followup(_session(), _lead(), "   ", TENANT, n=1)
    finally:
        patcher.stop()
...
@pytest.mark.asyncio
async def test_push_followup_always_passes_tenant_id_to_get_setting():
    reads = []
    post = AsyncMock(return_value=_response({"success": True}))
    patcher, _ = _client_patch(post)
    try:
        with patch.object(astro_bridge, "get_setting", new=_settings_stub(reads)):
            await astro_bridge.push_followup(_session(), _lead(), "And my career?", TENANT, n=1)
    finally:
        patcher.stop()
