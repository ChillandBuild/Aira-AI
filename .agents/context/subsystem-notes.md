...
### Operator Console — Monetization / Entitlements (2026-07-01)
- `tenants.enabled_features` (jsonb list) is the console-facing entitlement source; the sidebar + Feature Store read it. `tenant_subscriptions` holds plan ids + `ai_tier` + `mrr` (authoritative for billing). `create_client` writes `enabled_features = resolve_entitlements(plan) + pillar defaults` (messaging → whatsapp/inbound_leads/outbound_leads/analytics; telecalling → telecalling + dialer/scheduled/notes), seeds all 7 usage counters, and on failure cleans up ALL tenant rows (not just the auth user).
- **Metering is track-only**: `meter()` in `services/entitlements.py` wraps `increment_usage` and swallows every exception — it NEVER blocks a send. Hooks: `ai_reply.py` (`ai_reply`, only after a successful send), `broadcast_executor.py` (`message_sent`, per successful recipient), `calls.py` `telecmi_cdr` (`call_minute = ceil(duration/60)`, **TeleCMI only — NOT the SIM path**).
- **Feature Store**: metered rows (`ai_tier.*`, `tc_recording.summary/scoring`) are READ-ONLY. Toggling them would write meaningless keys into `enabled_features` without touching `tenant_subscriptions.ai_tier`; AI-tier changes must go through a subscription update, not `/features/toggle`.
- **`GET /operator/me` is load-bearing**: the operator login page AND `(console)/layout.tsx` guard both call it; deleting it locks everyone out of the console. The Fleet page must live under `app/operator/(console)/` to inherit the auth guard + sidebar.
- Pre-monetization tenants have NO `tenant_subscriptions` row → they show no plan/MRR until backfilled or recreated via the wizard.
