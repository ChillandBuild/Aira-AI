...
## Session Follow-ups
- **SIM Basic rollout checklist (Completed)**: The SIM manual analytics/status model, graceful push UX, provider-switch confirmation, provider-aware Live Agent Status, and mobile `tel:` dialer fix have been fully restored, fixed, and pushed to `main` (commit `91abfe2`). Testing issues (unit tests/Windows encoding) are resolved. In production, configure `VAPID_PUBLIC_KEY`/`VAPID_PRIVATE_KEY`/`VAPID_SUBJECT`, verify `pywebpush` is installed, and run database migration `121_sim_manual_call_statuses.sql` (if not already applied). Then manually verify Developer Console provider switching, provider-aware telecaller invite fields, desktop SIM handoff QR/send-to-mobile, mobile `tel:` dialer launch, and push deep-link opening `/dashboard/telecalling?lead_id=...`.
