    ...
    <div className="w-full max-w-[280px] mx-auto bg-white border border-[#e8e3db]/80 rounded-3xl p-4 shadow-sm flex flex-col gap-3">
      <div className="flex items-center gap-1 bg-[#faf8f5] border border-[#e8e3db]/80 rounded-2xl px-3 py-2.5">
        <input
          ...
          className="flex-1 bg-transparent font-manrope text-base font-bold text-[#292524] text-center tracking-wider outline-none"
        />
        {value.length > 0 && (
          <button
      ...
      <div className="grid grid-cols-3 gap-2">
        {buttons.map((btn) => (
          ...
          >
            <span className="font-manrope text-base font-extrabold text-[#292524] hover:text-inherit">{btn.digit}</span>
            <span className="font-manrope text-[7.5px] text-[#a8a29e] font-bold tracking-wider uppercase mt-0.5">{btn.sub || "\u00A0"}</span>
          </button>
        ))}
        ...
        <button
          ...
          className="col-span-3 mt-1 py-3.5 bg-gradient-to-r from-[#5b21b6] to-[#7c3aed] hover:from-[#5b21b6] hover:to-[#6d28d9] text-white rounded-2xl flex items-center justify-center gap-2 font-manrope text-sm font-extrabold shadow-[0_4px_12px_rgba(99,102,241,0.25)] hover:shadow-[0_6px_16px_rgba(99,102,241,0.4)] disabled:opacity-50 transition-all hover:scale-[1.01] active:scale-[0.99]"
        >
